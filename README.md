...
- [⚡ Using `tempo run` as a Standalone Command](#-using-tempo-run-as-a-standalone-command)
...
```bash
tempo create component --name button
```
...
COMMANDS:
   ...
   create, new, c, n  Generate a component or variant based on defined templates
   run, r             Process asset files into component templates
...
These definitions are used later by `tempo create` to generate real instances of components.
...
```bash
tempo create component --name button
tempo create variant --name outline --component button
```
...
Example: Running `tempo create component --name dropdown` will generate:

- components/dropdown/ (with .templ and .go files).

<details>
<summary><strong>Component Flags</strong> (<code>tempo create component</code>)</summary>
<dl>
...
<details>
<summary><strong>Variant Flags</strong> (<code>tempo create variant</code>)</summary>

This command shares all the flags from `create component`, plus:
...
Process asset files into templ component files
...
Whenever you update your CSS or JS files, simply run `tempo run` to propagate the changes.
...
> Check out [Using tempo run as a Standalone Command](#-using-tempo-run-as-a-standalone-command).
...
Remove temporary metadata generated by tempo run
...
- This command is useful if you need to reset the sync state before running `tempo run` again.

## ⚡ Using tempo run as a Standalone Command

The `run` command can be used **independently** of the scaffolding flow (`define`, `create`, `register`). All it requires is:
...
This makes `tempo run` a great option for developers who simply want to **synchronize CSS and JS assets** with their `.templ` files, without using the full scaffolding features.

### 💥 Use Cases for Standalone run
...
`tempo run` uses guard markers in `.templ` files to locate where CSS and JS should be injected.
...
> [!IMPORTANT]
> If no guard markers are present, tempo run will **skip** the file, ensuring only intended sections are updated.
